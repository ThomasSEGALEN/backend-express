...
module.exports = async function (api) {
    ...
    class UserControler {
        ...
        async findByEmail(email) {
            const verifiedUser = await bdd.query(`SELECT email, password FROM users WHERE email = "${email}"`);

            return verifiedUser;
        }

        async createUser(email, password, lastname, firstname) {
            const createUser = await bdd.query(`INSERT INTO users (email, password, lastname, firstname) VALUES ("${email}", "${password}", "${lastname}", "${firstname}")`);
    ...
    api.get('/users/create', async (req, res) => {
        const createUser = await user.createUser(req.query.email, req.query.password, req.query.lastname, req.query.firstname);
        let result;
        ...
        if (createUser.length > 0) {
            if (user[0]["email"] === req.query.email) {
                ...
                result = {success: false, errMessage: "password not enough characters"};
            } else if (createUser[0]["lastname"] && createUser[0]["firstname"]) {
                result = {success: true, errMessage: ""}
            }
        }
